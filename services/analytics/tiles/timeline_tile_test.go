...
func TestTimelineTile_Fetch(t *testing.T) {
	...
	t.Run("should include revenue in timeline", func(t *testing.T) {
		tc2 := di.NewTestContainer(t)
		defer tc2.Cleanup()

		_, project2 := fixtures.SetupAccountAndProject(t, tc2)
		tile2 := tiles.NewTimelineTile(tc2.ClickHouse)

		time.Sleep(500 * time.Millisecond)

		now := time.Now().UTC()
		visitorID := "visitor-revenue-test"
		...
		err = fixtures.InsertPaymentEventDirect(t, tc2, project2.ID, project2.OrganizationID, &visitorID, 10000, eventTime)
		require.NoError(t, err)
		...
		for _, dp := range response.Data {
			if dp.NumRevenue != nil && *dp.NumRevenue > 0 {
				foundRevenue = true
				assert.GreaterOrEqual(t, *dp.NumRevenue, int64(10000), "Should have revenue >= $100")
				t.Logf("Found revenue: $%.2f at %s", float64(*dp.NumRevenue)/100, dp.TimeBucket)
				break
			}
		}

		assert.True(t, foundRevenue, "Should find at least one data point with revenue")
	})
