...
func TestUniqueSessionsTile_Fetch(t *testing.T) {
	tc := di.NewTestContainer(t)
	defer tc.Cleanup()

	_, project := fixtures.SetupAccountAndProject(t, tc)
	tile := tiles.NewUniqueSessionsTile(tc.ClickHouse)

	time.Sleep(500 * time.Millisecond)

	t.Run("should count unique sessions in current and previous periods", func(t *testing.T) {
		now := time.Now().UTC()
		...
		assert.Equal(t, uint64(3), response.Count, "Should have 3 unique sessions in current period")
		assert.Equal(t, uint64(2), response.PreviousCount, "Should have 2 unique sessions in previous period")

		t.Logf("Current unique sessions: %d, Previous: %d", response.Count, response.PreviousCount)
	})
	...
	t.Run("should return zero when no events exist", func(t *testing.T) {
		...
		tile2 := tiles.NewUniqueSessionsTile(tc2.ClickHouse)
		...
		assert.Equal(t, uint64(0), response.Count, "Should have 0 unique sessions when no events")
		assert.Equal(t, uint64(0), response.PreviousCount, "Should have 0 previous unique sessions when no events")
	})
}
