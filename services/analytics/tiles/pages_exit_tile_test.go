...
func TestExitPagesTile_Fetch(t *testing.T) {
	tc := di.NewTestContainer(t)
	defer tc.Cleanup()

	_, project := fixtures.SetupAccountAndProject(t, tc)
	tile := tiles.NewExitPagesTile(tc.ClickHouse)

	time.Sleep(500 * time.Millisecond)

	t.Run("should aggregate sessions by exit page", func(t *testing.T) {
		now := time.Now().UTC()
		...
		events := []fixtures.EventInsertData{
			...
			},
			{
				VisitorID:          "visitor-4",
				SessionID:          "session-4",
				ClientTimestampUTC: now.Add(-4*time.Hour + 2*time.Minute),
				ProjectID:          project.ID,
				OrganizationID:     project.OrganizationID,
				UserAgent:          "Mozilla/5.0",
				IP:                 "127.0.0.1",
		...
		assert.GreaterOrEqual(t, len(response.Data), 3, "Should have at least 3 exit pages")

		var aboutPageCount uint64
		...
		assert.Equal(t, uint64(2), aboutPageCount, "/about should have 2 sessions ending on it")
		assert.Equal(t, uint64(1), contactPageCount, "/contact should have 1 session ending on it")
		assert.Equal(t, uint64(1), homePageCount, "/ should have 1 session ending on it")

		t.Logf("Exit pages data: %+v", response.Data)
	})
	...
	t.Run("should return empty when no sessions exist", func(t *testing.T) {
		...
		tile2 := tiles.NewExitPagesTile(tc2.ClickHouse)
		...
		require.NoError(t, err)
		require.NotNil(t, response)
		assert.Empty(t, response.Data, "Should have no exit pages when no sessions")
	})
}
